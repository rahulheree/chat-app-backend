# import asyncio
# from fastapi import APIRouter, Depends, HTTPException, status, Response, WebSocket, WebSocketDisconnect, File, UploadFile, Request
...
# async def start_session(request: Request, response: Response, user_in: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
...
# @cache(expire=120) # Cache for 2 minutes
# async def list_userspace_rooms(skip: int = 0, limit: int = 20, db: AsyncSession = Depends(get_db)):
...
#     print(f"[DEBUG] Incoming WS connect: room_id={room_id}, session_id={session_id}")

#     if not session_id:
#         print("[ERROR] No session_id cookie provided")
#         await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
#         return

#     async with AsyncSessionLocal() as db: # Manually create a session
#         user = await crud.get_user_by_session_id(db, session_id)
#         if not user:
#             print(f"[ERROR] No user found for session_id={session_id}")
#             await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
#             return
#         else:
#             print(f"[DEBUG] User resolved: id={user.id}, username={getattr(user, 'name', None)}")

#         membership = await crud.get_room_member(db, room_id, user.id)
#         if not membership:
#             print(f"[ERROR] User {user.id} not a member of room {room_id}")
#             await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="User not a member of this room")
#             return
#         else:
#             print(f"[DEBUG] User {user.id} is a member of room {room_id}")

#         await services.connection_manager.connect(websocket, room_id)
#         await services.redis_manager.add_active_user(room_id, user.id)
#         print(f"[INFO] User {user.id} connected to room {room_id}")
...
#                 print(f"[DEBUG] Received message from user {user.id}: {data}")

#                 message_data = schemas.MessageCreate.model_validate_json(data)
                
#                 if await services.is_spam(user.id, message_data.content):
#                     print(f"[WARN] Spam detected from user {user.id}: {message_data.content}")
...
#                 print(f"[INFO] Saved message {db_message.id} from user {user.id} in room {room_id}")

#                 await services.redis_manager.publish_message(
#                     room_id, schemas.Message.from_orm(db_message)
#                 )

#         except WebSocketDisconnect:
#             print(f"[INFO] WebSocket disconnected: user={user.id}, room_id={room_id}")
...
# @cache(expire=3600) # Cache invite links for 1 hour
...
#     # secure=False is for local HTTP development. In production (HTTPS), this must be True.
#     response.set_cookie(
#         key="session_id", 
#         value=session_id, 
#         httponly=True, 
...
async def start_session(request: Request, response: Response, user_in: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Reverted to samesite="lax" as requests are now same-origin via the Vite proxy.
    response.set_cookie(
        key="session_id", 
        value=session_id, 
        httponly=True, 
